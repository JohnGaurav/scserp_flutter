...
class ErrorLogBookScreen extends StatefulWidget {
  const ErrorLogBookScreen({super.key});

  @override
  State<ErrorLogBookScreen> createState() => _ErrorLogBookScreenState();
}

class _ErrorLogBookScreenState extends State<ErrorLogBookScreen> {
  int _rowsPerPage = 10;
  final TextEditingController _searchController = TextEditingController();
  late MyDataSource _source;
  bool _loading = true;
  List<dynamic> logbook = [];

  Future<void> apiData() async {
    http.Response apiResponse = await http.get(
      Uri.parse("$baseUrl/errorlogs"),
      headers: requestHeaders,
    );
    final decoded = jsonDecode(apiResponse.body);
    logbook = decoded["data"];

    setState(() {
      _source = MyDataSource(List<Map<String, dynamic>>.from(logbook));
      _loading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Error Logs'), elevation: 5.0),
                    ...
                    child: SizedBox(
                      ...
                      child: PaginatedDataTable2(
                        ...
                        columns: const [
                          ...
                          DataColumn(label: Text('Error Detail')),
                        ],
                        source: _source,
                        sortColumnIndex: 1,
  ...
  void search(String query, {bool matchAll = false}) {
    ...
    if (q.isEmpty) {
      ...
      _filteredData = _allData.where((row) {
        final title = _normalize(row['detail']?.toString() ?? '');
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow.byIndex(
      ...
      cells: [
        ...
        DataCell(Text(apijson['detail'].toUpperCase())),
      ],
    );
  }
