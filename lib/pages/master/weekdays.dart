...
class _WeekdaysScreenState extends State<WeekdaysScreen> {
  ...
  Future<void> apiData() async {
    http.Response apiResponse = await http.get(
      Uri.parse("$baseUrl/weekdays"),
      headers: requestHeaders,
    );
    final decoded = jsonDecode(apiResponse.body);
    weekdaysList = decoded["data"];

    setState(() {
      _source = MyDataSource(List<Map<String, dynamic>>.from(weekdaysList));
      _loading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Weekdays'), elevation: 5.0),
